/* tslint:disable */
/**
 * This file was automatically generated by json-schema-to-typescript.
 * DO NOT MODIFY IT BY HAND. Instead, modify the source JSONSchema file,
 * and run json-schema-to-typescript to regenerate this file.
 */

export type PartnersAgreementContractFunctions1 =
  | []
  | [PartnersAgreementContractFunctions]
  | [PartnersAgreementContractFunctions, PartnersAgreementContractEvents];
export type SWEvent = {
  event: PartnersAgreementContractEventType;
  args: any;
}[];

export interface PartnersAgreementContractFunctions {
  activities: () => Promise<string>;
  addNewContractAddressToAgreement: (
    contractAddress: string
  ) => Promise<{wait: () => Promise<PartnersAgreementContractEvents>}>;
  addURL: (_url: string) => Promise<{wait: () => Promise<PartnersAgreementContractEvents>}>;
  commitmentLevel: () => Promise<number>;
  communityAddress: () => Promise<string>;
  createActivity: (_type: number, _uri: string) => Promise<{wait: () => Promise<PartnersAgreementContractEvents>}>;
  deployActivities: (_factory: string) => Promise<{wait: () => Promise<PartnersAgreementContractEvents>}>;
  finilizeTask: (_activityId: number) => Promise<{wait: () => Promise<PartnersAgreementContractEvents>}>;
  getActivitiesAddress: () => Promise<string>;
  getAgreementData: () => Promise<undefined>;
  getAllMembers: () => Promise<string[]>;
  getImportedAddresses: () => Promise<string[]>;
  getSkillWalletAddress: () => Promise<string>;
  getURLs: () => Promise<string[]>;
  interactionNFT: () => Promise<string>;
  interactionNFTFactory: () => Promise<string>;
  isActive: () => Promise<boolean>;
  isURLListed: (_url: string) => Promise<boolean>;
  onERC1155BatchReceived: () => Promise<string>;
  onERC1155Received: () => Promise<string>;
  onERC721Received: () => Promise<string>;
  owner: () => Promise<string>;
  partnersContracts: () => Promise<string>;
  removeURL: (_url: string) => Promise<{wait: () => Promise<PartnersAgreementContractEvents>}>;
  rolesCount: () => Promise<number>;
  setActivities: (
    _activity: string,
    _interactionNFT: string
  ) => Promise<{wait: () => Promise<PartnersAgreementContractEvents>}>;
  supportsInterface: (interfaceId: string) => Promise<boolean>;
  takeTask: (_activityId: number) => Promise<{wait: () => Promise<PartnersAgreementContractEvents>}>;
  transferInteractionNFTs: (
    user: string,
    amountOfInteractions: number
  ) => Promise<{wait: () => Promise<PartnersAgreementContractEvents>}>;
  urls: () => Promise<string>;
  version: () => Promise<number>;
}
export interface PartnersAgreementContractEvents {
  events: SWEvent;
}

export enum PartnersAgreementContractEventType {
  PartnersContractAdded = "PartnersContractAdded",
  UrlAdded = "UrlAdded"
}
